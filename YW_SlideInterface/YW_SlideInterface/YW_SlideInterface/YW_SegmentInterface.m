...
static CGFloat const defaultItemH = 50;
...
{
    if (!_titleScrollView) {
        _titleScrollView = [[YW_TitleScrollView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, defaultItemH)];
...
{
    if (!_childScrollView) {
        _childScrollView = [[YW_ChildScrollView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_titleScrollView.frame), self.frame.size.width, self.frame.size.height - _titleScrollView.frame.size.height)];
...
-(void)intoTitlesArray:(NSMutableArray *)titlesArray hostController:(UIViewController *)hostController
{
    ...
    _childScrollView.contentSize = CGSizeMake(self.frame.size.width * titlesArray.count, 0);
    ...
    _titleScrollView.tabItemClickBlock = ^(int currentNum) {
