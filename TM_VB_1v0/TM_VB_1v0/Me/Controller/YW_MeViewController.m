...
@interface YW_MeViewController () <MJCSlideSwitchViewDelegate, UIScrollViewDelegate, UISearchBarDelegate, UITextFieldDelegate, SegmentInterfaceDelegate, UIGestureRecognizerDelegate>
{
...
{
    if (!_menuTabArr) {
        NSString *path = [[NSBundle mainBundle] pathForResource:@"MeMenuTab" ofType:@"plist"];
...
- (void)viewDidLoad {
    ...
    [[YW_MenuSingleton shareMenuInstance] setViewTitle:@"Me"];
...
- (void)setupNavigationBar
{
    ...
    [menuBar setUpMenuWithTitleArr:[self menuArr:[self.menuTabArr objectForKey:@"MyProfile"]]];
...
-(void)clickMenuItem:(YW_MenuBarLabel *)label
{
    ...
    if ([self.tabTitleArr[currentChildIndex] isEqualToString:@"MyProfile"]) {
        int index = (int)[[self menuArr:[self.menuTabArr objectForKey:@"MyProfile"]] indexOfObject:label.text];
        switch (index) {
            case 0:
    ...
    if ([self.tabTitleArr[currentChildIndex] isEqualToString:@"ThisAccount"]) {
        int index = (int)[[self menuArr:[self.menuTabArr objectForKey:@"ThisAccount"]] indexOfObject:label.text];
        switch (index) {
            case 0:
                [self showSlideMenu:YW_ShowMenuFromLeft withViewController:[YW_SubMenuViewController class]];
                break;
...
- (void)setUpTabBar
{
    ...
    self.tabTitleArr = [NSMutableArray arrayWithObjects:@"MyProfile", @"ThisAccount", @"Help", nil];
