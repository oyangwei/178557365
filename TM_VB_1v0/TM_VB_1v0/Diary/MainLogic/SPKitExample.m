...
#warning TODO: CHANGE TO YOUR ACTUAL Input View Plugin
    ...
    if ([aConversationController.messageInputView isKindOfClass:[YWMessageInputView class]]) {
        ...
        if ([aConversationController.conversation isKindOfClass:[YWP2PConversation class]]) {
            /// The pass-through message currently supports only single chat sessions
            /// This function is used only as sample code
...
/**
 *  设置编辑模式下，转发按钮被点击后的事件，以及打开合并转发聊天记录详情的事件
 */
- (void)exampleSetForwardBlockOfEditModeToConversationController:(YWConversationViewController *)aConversationController
{
    __weak typeof(self) weakSelf = self;
    __weak typeof(aConversationController) weakController = aConversationController;
    
    void (^presentSelectContactBlock)(void(^)(NSArray *), UIViewController *) = ^ (void(^aDoneBlock)(NSArray *), UIViewController *aFromController) {
#if HAS_CONTACTLIST
        ...
        [aFromController presentViewController:naviController
                                      animated:YES
                                    completion:NULL];
        
        [contactListController setDoneBlock:^(SPContactListController *aController, NSArray *aPersonIDs) {
            aDoneBlock(aPersonIDs);
        }];
#else
        dispatch_async(dispatch_get_main_queue(), ^{
            aDoneBlock(@[@"jiakuipro003"]);
        });
#endif
    };
    
    [aConversationController setRespectivelyForwardBlock:^(NSArray< id<IYWMessage> > *aMessages, YWConversationViewController *aFromController) {
        presentSelectContactBlock(^ void (NSArray *aPersonIDs) {
            [YWIndicator showTopToastTitle:@"逐条转发" content:nil userInfo:nil withTimeToDisplay:1.f andClickBlock:NULL];
            for (NSString *personId in aPersonIDs) {
                YWPerson *person = [[YWPerson alloc] initWithPersonId:personId];
                YWP2PConversation *conv = [YWP2PConversation fetchConversationByPerson:person creatIfNotExist:YES baseContext:weakSelf.ywIMKit.IMCore];
                for (id<IYWMessage> msg in aMessages) {
                    [conv asyncForwardMessage:msg progress:NULL completion:NULL];
                }
            }
            
            [weakController setEditingFlag:NO];

        }, aFromController);
    }];
    
    [aConversationController setMergeForwardBlock:^(YWMessageBodyCustomizeInternal *aMergedBody, YWConversationViewController *aFromController) {
        if (aMergedBody == nil) {
            return;
        }
        
        presentSelectContactBlock(^ void (NSArray *aPersonIDs) {
            [YWIndicator showTopToastTitle:@"合并转发" content:nil userInfo:nil withTimeToDisplay:1.f andClickBlock:NULL];
            
            for (NSString *personId in aPersonIDs) {
                YWPerson *person = [[YWPerson alloc] initWithPersonId:personId];
                YWP2PConversation *conv = [YWP2PConversation fetchConversationByPerson:person creatIfNotExist:YES baseContext:weakSelf.ywIMKit.IMCore];
                [conv asyncSendMessageBody:aMergedBody progress:NULL completion:NULL];
            }
    ...
    /// 设置合并转发消息的点击事件
    [aConversationController setCombineChatBubbleClickedBlock:^(YWConversationViewController *aCombineController) {
        [weakController.navigationController pushViewController:aCombineController animated:YES];
    }];
}
...
- (void)exampleSetMenuForEditModeToConversationController:(YWConversationViewController *)aConversationController
{
    ...
    [aConversationController setEditModeActionsBlock:^NSArray<YWMoreActionItem *> *(YWConversationViewController *aFromController) {
        ...
        [result addObject:[weakController fetchEditModeItemForForward]];
        return result;
    }];
    
    /// 设置编辑模式下，转发按钮被点击后的事件
    [self exampleSetForwardBlockOfEditModeToConversationController:aConversationController];
}
...
- (void)exampleListenNewMessage
{
    [[self.ywIMKit.IMCore getConversationService] addOnNewMessageBlockV2:^(NSArray *aMessages, BOOL aIsOffline) {
        ...
        /// You can play the prompts here as needed
        
        /// Show the pass-through message
...
#warning TODO: JUST RETURN IF NO NEED TO ADD Custom Menu When Preview Image OR CHANGE TO YOUR ACTUAL METHOD
    ...
    [self.ywIMKit setPreviewImageMessageBlockV3:^(id<IYWMessage> aMessage, YWConversation *aOfConversation, NSDictionary *aFromUserInfo) {
        ...
        if (![aOfConversation isKindOfClass:[YWCombinedForwardConversation class]]) {
            /// 合并聊天记录的消息，不能被转发
